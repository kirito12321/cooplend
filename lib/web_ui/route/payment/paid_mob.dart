                                                                                    ...
                                                                                    await myDb.collection('loans').doc('${prefs.data!.getString('coopId')}_${data[index]['payerId']}_${data[index]['loanId']}').get().then((value) async {
                                                                                      if (double.parse(data[index]['amount'].toStringAsFixed(2)) >= double.parse(data[index]['monthlyPay'].toStringAsFixed(2))) {
                                                                                        ...
                                                                                        await myDb.collection('loans').doc('${prefs.data!.getString('coopId')}_${data[index]['payerId']}_${data[index]['loanId']}').collection('tenure').doc(data[index]['tenureId']).get().then((tenures) async {
                                                                                          ...
                                                                                          if (DateTime.now().difference(data[index]['dueDate'].toDate()).inHours > 0) {
                                                                                            await myDb.collection('coops').doc(prefs.data!.getString('coopId')).get().then((cp) => myDb.collection('loans').doc(value.id).collection('tenure').doc((int.parse(data[index]['tenureId']) + 1).toString()).get().then((ten) async {
                                                                                                  await myDb.collection('loans').doc(value.id).collection('tenure').doc((int.parse(data[index]['tenureId']) + 1).toString()).update({
                                                                                                    'penalty': cp.data()!['penalty'] * tenures.data()!['monthInterest'],
                                                                                                    'payment': double.parse((cp.data()!['penalty'] * tenures.data()!['monthInterest']).toStringAsFixed(2)) + ten.data()!['payment'],
                                                                                                  });
                                                                                                  await myDb.collection('loans').doc('${prefs.data!.getString('coopId')}_${data[index]['payerId']}_${data[index]['loanId']}').update({
                                                                                                    'totalPenalty': value.data()!['totalPenalty'] + (cp.data()!['penalty'] * ten.data()!['monthInterest']),
                                                                                                  });
                                                                                                }));
                                                                                          }
                                                                                        });

                                                                                        await myDb.collection('payments').doc(data[index].id).update({
              ...
              default:
                return StreamBuilder(
                  stream: myDb
                      ...
                      // .where('payStatus', whereIn: ['paid', 'paid overdue'])
  ...
  DataRow getRow(int index) {
    var list = DataRow(
      cells: [
        ...
        DataCell(
          Text(
            DateFormat('MMM d, yyyy').format(_data[index]["dueDate"].toDate()),
            style: GoogleFonts.montserrat(
              fontSize: 15,
              fontWeight: FontWeight.w800,
              color: Colors.red[800],
    ...
    if (_data[index]['invoiceNo'].toString().contains(searchStr)) {
      return list;
    }
